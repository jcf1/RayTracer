...
void App::makeGUI() {
    ...
    renderPane->addDropDownList("Resolution", Array<String>{"1x1", "320x200", "640x400"}, &m_resolution);

    renderPane->addCheckBox("Fixed Primitives", &m_fixedPrimitives);

    renderPane->addCheckBox("MultiThreaded", &m_multiThreaded);
        
    renderPane->addNumberBox("Indirect Rays", &m_indirectRaysPP, "", GuiTheme::NO_SLIDER, 0, 2048);
    
    renderPane->addButton("Generate", [this](){
        drawMessage("Rendering Image.");
        onRender();
    });
}
...
void App::onRender(){
    RayTracer tracer(m_indirectRaysPP, m_multiThreaded, m_fixedPrimitives);
    shared_ptr<Image> newImage;
    if(m_resolution == 0){newImage = Image::create(1, 1, ImageFormat::RGB8());}
    if(m_resolution == 1){newImage = Image::create(320, 200, ImageFormat::RGB8());}
    if(m_resolution == 2){newImage = Image::create(640, 400, ImageFormat::RGB8());}
    String time = tracer.traceImage(scene(), activeCamera(), newImage);
    const String name = "result.png";
    newImage->save(name);
    show(newImage, time);
}
